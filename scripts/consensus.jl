...
sec_str_primer = config["sec_str_primer"] #defining forward primer so it can be trimmed off 
...
trimmed_collection = [double_primer_trim(s,to_trim,sec_str_primer) for s in seq_collection]; #the forward primer is used during Index-demux.jl so it also needs to be trimmed off here
