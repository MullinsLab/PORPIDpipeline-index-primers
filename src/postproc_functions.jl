...
function H704_init_template_proc(fasta_collection, panel_file,
    ...
    if length(realigned_seqs) == 0
		println("------------------------------------------------------------------------------")
		println("")
    	@error "No sequences from $(split(basename(f),".fast")[1]) pass the panel threshold. Check sequences in
    	contam_passed folder and alter panel_thresh in Snakefile if needed"
    	println("")
    	println("------------------------------------------------------------------------------")
	else
    ...
    fig = ali2MDS(realigned_seqs,
            realigned_sizes,
            post_APOBEC)
    title("P(APOBEC|mutations): $(split(basename(f),".fast")[1])")
